					...
					Containers: []corev1.Container{
						{
							...
							Command: []string{
								...
								for i in $(seq 0 $((${INITIAL_CLUSTER_SIZE} - 1))); do
									while true; do
										echo "Waiting for ${SET_NAME}-${i} to come up"
										ping -W 1 -c 1 ${SET_NAME}-${i} > /dev/null && break
										sleep 1s
									done
								done
								...
								if [ -e /var/run/etcd/default.etcd ]; then
									...
									# re-join member
									ETCDCTL_ENDPOINT=$(eps) etcdctl member update ${member_id} http://${HOSTNAME}:2380 | true
								...
								if [ "${SET_ID}" -ge ${INITIAL_CLUSTER_SIZE} ]; then
									...
									etcdctl member add ${HOSTNAME} http://${HOSTNAME}:2380 | grep "^ETCD_" > /var/run/etcd/new_member_envs
