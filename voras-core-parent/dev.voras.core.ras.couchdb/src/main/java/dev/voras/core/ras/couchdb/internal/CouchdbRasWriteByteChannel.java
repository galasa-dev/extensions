...
public class CouchdbRasWriteByteChannel implements SeekableByteChannel {
	...
	CouchdbRasWriteByteChannel(CouchdbRasStore couchdbRasStore, Path remotePath, ResultArchiveStoreContentType remoteContentType, Set<? extends OpenOption> options, FileAttribute<?>[] attrs) throws IOException {
		...
		//TODO put these all in the same /tmp/cirillo dir so it is easy to cleanup
		cachePath = Files.createTempFile("cirillo_couchdb", "temp");

		// TODO are we going to provide other options?  StandardOpenOption.TRUNCATE_EXISTING   need to standardise across the board
		cacheByteChannel = Files.newByteChannel(cachePath, StandardOpenOption.WRITE, StandardOpenOption.TRUNCATE_EXISTING);
	}
	...
	public void close() throws IOException {
		...
		HttpPut request = new HttpPut(this.couchdbRasStore.getCouchdbUri() + "/cirillo_artifacts/" + this.couchdbRasStore.getArtifactDocumentId() + "/" + encodedRemotePath);
		...
		try (CloseableHttpResponse response = this.couchdbRasStore.getHttpClient().execute(request)) {
			...
			PutPostResponse putPostResponse = this.couchdbRasStore.getGson().fromJson(EntityUtils.toString(entity), PutPostResponse.class);
			...
			logger.info("Stored artifact " + this.remotePath + ", length=" + Files.size(cachePath) + ", contentType=" + this.remoteContentType.value());
		} catch(Exception e) {
			throw new IOException("Unable to store artifact attachment", e);
		} finally {
			try {
				Files.delete(cachePath);
			} catch(Exception e) {} //*** Hide any delete problems
		}
	}
