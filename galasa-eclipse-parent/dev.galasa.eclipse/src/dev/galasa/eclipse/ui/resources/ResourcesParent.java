...
//		}, 30000, 30000);
	...
	public synchronized void propertyUpdate(PropertyUpdate propertyUpdate) {
		...
		//*** Resource are based on the .run property prefix,  so if it is a .run,    find the resource and update/create it
		if (parts[parts.length - 1].equals("run")) {
			...
			for(int i = 1; i < parts.length; i++) {
				...
				if (resource != null) {
					...
					resource.propertyUpdate(new PropertyUpdate(remainderName, propertyUpdate.getValue(), propertyUpdate.getType()));
					update(resource);
					return;
				}
			}
			...
			if (propertyUpdate.getType() != Type.DELETE) {
				...
				pendingProperties.add(new PropertyUpdate(remainderName, propertyUpdate.getValue(), propertyUpdate.getType()));
				return;
			}
