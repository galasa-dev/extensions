...
public class SubmitTestRunsJob extends Job {
	...
	public SubmitTestRunsJob(TestStream testStream, 
			List<TestClass> selectedClasses, 
			boolean trace, 
			String requestorId, 
			IFile overridesFile, 
			boolean includeGlobalOverrides, 
		...
		try {
			...
			for(TestClass klass : selectedClasses) {
				IRun run = runs.submitRun("request", // TODO provide option 
						...
						klass.getBundle(), 
						klass.getName(), 
						null,   // TODO provide a means to enter a group name or default 
						mavenRepository, 
						obr, 
						testStream.getId(), 
						false, 
